...
## Preliminary Check on the BCG Spectral Cubes
  Before we start, we need to first categorize our BCG into two groups: detected and non-detected. To do so, we can use the **CARTA** software and look at the spectrum window to do a visual assessment.

  From here, we can use **CASA** to do reimaging for the desired bin values for velocity channel for the spectral cube. The reimaged products are now ready for the data processing.
---
